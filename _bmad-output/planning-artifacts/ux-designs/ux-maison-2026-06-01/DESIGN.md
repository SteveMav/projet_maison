...
sources:
  ...
  - imports/design/stitch-imports/detail-ngaliema/property-01.jpg
  - imports/design/stitch-imports/detail-ngaliema/property-02.jpg
  - imports/design/stitch-imports/detail-ngaliema/property-03.jpg
  - imports/design/stitch-imports/detail-ngaliema/property-04.jpg
colors:
  surface-base: '#F7F4EE'
...
components:
  ...
  status-chip:
    radius: '{rounded.full}'
    note: 'Text plus icon where needed; color never carries meaning alone.'
  search-form:
    background: '{colors.surface-raised}'
    radius: '{rounded.md}'
...
Reference bundle: the [source map](imports/design/README.md), [interactive proposal](imports/design/index.html), [prototype styles](imports/design/styles.css), [prototype behavior](imports/design/app.js), and [Stitch reconciliation](imports/design/STITCH-REVIEW.md) show the working direction. They illustrate the contract; they do not replace it.
...
Load-bearing foreground and background combinations must be verified against WCAG 2.2 AA before implementation: primary ink on base and raised surfaces, inverse ink on accent buttons, secondary ink on raised surfaces, error on error-soft, and verified-state ink on trust-soft.
...
The inspected Stitch [screenshot](imports/design/stitch-imports/detail-ngaliema/screen.png) and its generated [HTML export](imports/design/stitch-imports/detail-ngaliema/screen.html) are useful composition references:
...
- Pills: `{rounded.full}` only for filters, status chips, and compact badges.
...
The imported Stitch placeholders [01](imports/design/stitch-imports/detail-ngaliema/property-01.jpg), [02](imports/design/stitch-imports/detail-ngaliema/property-02.jpg), [03](imports/design/stitch-imports/detail-ngaliema/property-03.jpg), and [04](imports/design/stitch-imports/detail-ngaliema/property-04.jpg) are useful for layout review but skew upscale. The production design agent should deliberately test ordinary, credible rental inventory as well.
...
|---|---|
| Filter chip | Neutral pill by default; `{colors.accent-soft}` when selected; always retain a visible text label. |
| Listing-shaped skeleton | Match the card image block and text rhythm with `{colors.surface-soft}` tonal placeholders. |
| Gallery | Use `{rounded.sm}` on images, a `4:3` main frame, horizontal thumbnails, and a compact count overlay. |
| Availability block | Keep status and last-update copy together on a quiet tonal surface. Warning states use `{colors.warning-soft}` plus an icon and text. |
| Commissionnaire block | Use a compact identity row with hairline border and no decorative elevation. |
| Report action | Use `{colors.error}` text as a visually secondary action; do not style it as a competing CTA. |
| Lightweight identification | Raised modal or sheet with concise explanation, fields, explicit consent, privacy reassurance, and one primary action. |
| Recoverable Lead error | Inline `{colors.error-soft}` surface with `{colors.error}` text and a retry action; preserve entered values. |
| Inventory row | Compact row with thumbnail, listing identity, freshness or moderation status, and hairline divider. |
| Lead row | Compact row with tenant identity, originating listing, timestamp, and optional status. |
| Publication stepper | Short mobile-friendly sequence; active step uses `{colors.accent}`, inactive steps remain quiet. |
| Verification checklist | Operational list with explicit checked states, text labels, hairline dividers, and recorded actor context. |
